...
APP_NAME = "HorseProno Quinté V4.3 — Outsider Detector"
PMU_BASE_URL = "https://online.turfinfo.api.pmu.fr/rest/client/1"
REQUEST_TIMEOUT = 25
HEADERS = {
    "User-Agent": "HorsePronoQuinteV43/4.3 (+https://streamlit.io)",
...
st.title("🏇 HorseProno Quinté V4.3 — Outsider Detector")
st.caption(
    "Le programme PMU complet de la journée est chargé automatiquement. "
    "La sélection officielle reste la V4.2. En PLAT, la V4.3 analyse en parallèle les outsiders absents de V2 et V3 en mode shadow, sans modifier automatiquement le Top 7 tant que la validation forward est insuffisante."
)
...
def run_prediction(history: pd.DataFrame, race: pd.DataFrame) -> pd.DataFrame:
    ...
    return rank_quinte_v43(history, race, artifact, ranker, outsider)
...
try:
    ...
    if st.button("🏇 Analyser cette course", type="primary"):
        ...
        st.subheader("🎯 Sélection officielle V4.2 + détecteur outsider V4.3")
        ...
        st.caption(
            ...
            "Cette version est optimisée pour la présence des chevaux de l'arrivée dans les 7, pas pour l'ordre exact."
        )
        ...
        display = ranked[
            [
                ...
                "v42_edge_score", "v43_outsider_probability", "v43_outsider_candidate",
